...
execute at @a[scores={po.use.smoke=1..}] run summon minecraft:area_effect_cloud ~ ~ ~ {Duration:110,Tags:["Fumee"]}
execute as @a[scores={po.use.smoke=1..}] run scoreboard players set @s po.use.smoke 0
...
execute as @a[scores={po.use.assassinate=1..}] run execute at @s run execute at @a[limit=1,tag=!mort,sort=random,distance=1..] run tp @s ^-1 ^0.2 ^ facing entity @a[distance=..2,limit=1] feet
execute as @a[scores={po.use.assassinate=1..}] run scoreboard players set @s po.use.assassinate 0