#canne à pêche
execute at @e[type=pitchout:fishing_bobber,] run execute as @a[distance=..2,tag=!mort,scores={fishing=0}] run tag @p[scores={fishing=1..}] add grap
...
execute as @a[tag=grabber] run execute at @e[type=pitchout:fishing_bobber] run execute as @a[distance=..2,tag=!mort,scores={fishing=0}] run tag @s add grabbed
execute if entity @a[scores={DetectMort=1..,timer=2},tag=grabbed] run scoreboard players add @a[tag=grabber] advBarroud 1
#------------------------------------------------------------------------------------------

execute at @e[type=pitchout:fishing_bobber] run execute as @e[distance=..2,type=!pitchout:fishing_bobber,type=!pitchout:area_effect_cloud,type=!pitchout:armor_stand,tag=!mort] run tp @s @p[scores={fishing=1..}]
