...
execute if entity @a[team=guerrier,scores={bouclier=1..}] run item replace entity @a[team=guerrier,scores={bouclier=1..}] hotbar.7 with minecraft:totem_of_undying[minecraft:lore=['[{"text":"Impossible pour vous d être repoussé"}]','[{"text":"par quoi que ce soit durant "},{"text":"5 secondes","color":"red"}]','[{"text":"(C est le pouvoir des cookies)","color":"aqua"}]'],minecraft:attribute_modifiers={modifiers:[{type:"minecraft:generic.knockback_resistance",name:"",amount:150,operation:"add_value",id:"generic.knockback_resistance"}]},minecraft:enchantments={levels:{"minecraft:fortune":3},show_in_tooltip:0b},minecraft:unbreakable={},minecraft:custom_name='[{"text":"INVINCIBILITE","color":"red","bold":true},{"text":"","color":"red","bold":false}]',minecraft:hide_additional_tooltip={}] 1
execute at @a[scores={bouclier=1..}] run kill @e[type=minecraft:arrow,nbt={ShotFromCrossbow:0b},distance=..3]

execute at @a[scores={bouclier=1..}] run execute as @a[distance=..4,scores={bouclier=0},gamemode=!spectator] run effect give @s minecraft:slowness 2 2
...
execute at @a[scores={bouclier=1..}] run execute at @e[distance=..4,scores={bouclier=0},gamemode=!spectator] run particle minecraft:elder_guardian ~ ~ ~ 0.1 0.1 0.1 2 10 force

execute if entity @a[nbt={Inventory:[{id:"minecraft:totem_of_undying"}]},team=guerrier] run scoreboard players add @a[nbt={Inventory:[{id:"minecraft:totem_of_undying"}]}] invul 1
execute as @a[scores={invul=140..}] run scoreboard players set @s bouclier 0
execute as @a[scores={invul=140..}] run clear @s minecraft:totem_of_undying
...
execute at @e[type=minecraft:firework_rocket,nbt={ShotAtAngle:1b}] run execute as @e[tag=!rocket,type=!minecraft:firework_rocket,type=!area_effect_cloud,type=!armor_stand,distance=..5,tag=!mort] run tp @s ~1 ~ ~1
