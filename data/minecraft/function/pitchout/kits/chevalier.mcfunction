...
execute as @a[scores={charge=1..,timeCharge=..45}] run effect give @s minecraft:speed 1 18 true
execute at @a[scores={charge=1..}] run particle minecraft:cloud ~ ~ ~ 0.1 0.4 0.1 0.2 9 force
...
execute at @e[tag=CHARGE,type=area_effect_cloud] run execute as @a[tag=!chargez] run tp @s[distance=..2,tag=!chargez,type=!item,type=!armor_stand,type=!arrow,tag=!mort,type=!minecraft:area_effect_cloud] ^ ^0.2 ^4 facing entity @e[tag=CHARGE,type=area_effect_cloud,sort=nearest,limit=1] feet
#------------------------------------------------------------------
#bouclier

execute as @a[scores={protect=1..}] run scoreboard players add @s timeProtect 1
execute as @a[scores={protect=1..,timeProtect=1..2}] run item replace entity @s weapon.offhand with minecraft:shield
execute at @a[scores={protect=1..,timeProtect=1..2}] run particle minecraft:flash ~ ~ ~ 0.1 0.4 0.1 0.2 10 force

execute as @a[scores={protect=1..,timeProtect=400..}] run item replace entity @s weapon.offhand with minecraft:air
...
execute as @e[type=minecraft:area_effect_cloud,tag=RES] run execute store result score @s timeBlock run data get entity @s Age 1
...
execute at @e[type=minecraft:area_effect_cloud,tag=RES,scores={timeBlock=2397..}] run fill ~ ~ ~ ~ ~ ~ air replace minecraft:honeycomb_block

#------------------------------------------------------------------------
#autel de resurection

execute if entity @a[scores={resu=1..}] run execute at @e[type=minecraft:allay] run summon minecraft:area_effect_cloud ~ ~ ~ {Duration:2400,Tags:["RES"]}
execute if entity @a[scores={resu=1..}] run execute at @e[type=minecraft:area_effect_cloud,tag=RES] run fill ~ ~ ~ ~ ~ ~ minecraft:bedrock keep
execute if entity @a[scores={resu=1..}] run execute at @e[type=minecraft:area_effect_cloud,tag=RES] run fill ~ ~1 ~ ~ ~1 ~ minecraft:amethyst_cluster keep
execute if entity @a[scores={resu=1..}] run kill @e[type=minecraft:allay]
